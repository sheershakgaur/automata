...
{   x:
    ...
    cout << "Enter your Context-Free S->aAB|bCB|SSGrammar in GNF (enter 'x' to stop): ";
    cout << endl << endl;
    ...
    for(;;)
    {
        ...
        if(hold == "x") break;
            else
            {
                ...
                {
                    prod += *iter; //inserts everything to the right of the arrow
                    ++iter;
                }
...
void convert()
{
    ...
    for(int i = 0; i < grammar.size(); i++) //rules = everything to the right of the arrow
    {
        ...
        if(grammar[i].size() > 2)
        {
            ...
            for(int c = 1; c < grammar[i].size(); c++) 
            {
                if((c > 1) && (grammar[i][c].at(0) != grammar[i][c - 1].at(0)))
                {
                    ...
                    {
                        cout << "\nd(q1, " << grammar[i][1].at(0) << ", " << grammar[i][0] << ") = {(q1, "<<grammar[i][c].at(1)<<")}\n";
